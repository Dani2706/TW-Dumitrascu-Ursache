...
@WebServlet("/api/user-listings-view-count")
public class GetUserListingsViewCount extends HttpServlet {
    private static final Logger logger = LoggerFactory.getLogger(GetUserListingsViewCount.class);
    ...
    public void init() throws ServletException {
        ...
        logger.info("GetUserListingsViewCount controller initialized");
    }

    @Override
    protected void doGet(HttpServletRequest request, HttpServletResponse response) {
        logger.debug("Received request for user listings view count");
        String authHeader = request.getHeader("Authorization");
        try {
            String token = this.jwtUtil.verifyAuthorizationHeader(authHeader);

            String jsonString = propertyService.getUserListingsViewCount(token);
            ...
            logger.error("Error retrieving user listings view count: {}", e.getMessage());
